...
trait WPJAM_QrcodeSignupTrait{
	...
	public static function bind_form($echo=true){
		...
		if($openid){
			$third_user	= self::get_third_user($openid);
		...
		if($third_user){
			$form	.=  '<p>绑定的微信账号是：</p>';
			$form	.=  '<p>昵称：'.$third_user['nickname'].'<br />地区：'.$third_user['province'].' '.$third_user['city'].'</p>';

			$avatar_field	= self::get_avatar_field();
			$avarar_url		= $third_user[$avatar_field];
			
			$form	.=  '<p><img src="'.str_replace('/132', '/0', $avarar_url).'" width="160" /></p>';
