...
class WEIXIN_Signup extends WPJAM_Signup{
	...
	public static function scan_reply($keyword){
		global $weixin_reply;
		$message	= $weixin_reply->get_message();
		$scene		= $message['EventKey'] ?? '';

		if($scene && self::get_reply($scene)){
		}else{
			$weixin_reply->scan_reply($keyword);
		}
	}

	public static function subscribe_reply($keyword){
		global $weixin_reply;
	...
	public static function get_reply($scene){
		global $weixin_reply;
	...
	protected static function get_email($openid){
		return wp_slash($openid).'@'.self::$appid.'.weixin';
	}

	protected static function get_cache_key($key){
		return 'weixin:'.self::$appid.':'.$key;
	}
