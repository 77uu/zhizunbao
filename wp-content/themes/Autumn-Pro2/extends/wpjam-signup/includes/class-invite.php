...
class WPJAM_Invite{
	...
	private static function delete($invite_key){
		$invites	= self::get_invites();
	...
	private static function get_invites(){
		if(is_multisite()){
			$invites	= get_site_option('wpjam_user_invites');
		}else{
	...
	private static function update_invites($invites){
		if(is_multisite()){
			return update_site_option('wpjam_user_invites', $invites);
		}else{
			return update_option('wpjam_user_invites', $invites);
		}
	}
}