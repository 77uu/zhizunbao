...
add_filter('wpjam_pages', function ($wpjam_pages){
	...
	foreach ($template_types as $type=>$tt){
		...
		$wpjam_pages['templates']['subs']['wpjam-'.$type] =[
			...
			'query_args' 	=> ['post_id'],
			'page_file'		=> WPJAM_CONTENT_TEMPLATE_PLUGIN_DIR .'admin/pages/'.$type.'.php'
		];
	}
	...
	foreach($post_types as $post_type => $pt_obj ){
		$wpjam_pages[$post_type.'s']['subs']['wpjam-'.$post_type.'-template-setting'] =[
			...
			'page_file'		=> WPJAM_CONTENT_TEMPLATE_PLUGIN_DIR .'admin/pages/singular.php'
		];
	}
	...
	$wpjam_pages['templates']['subs']['wpjam-template-setting'] =[
		...
		'function'		=> 'option',
		'option_name'	=> 'wpjam-content-template',
		'page_file'		=> WPJAM_CONTENT_TEMPLATE_PLUGIN_DIR .'admin/pages/settings.php'
	];
...
add_filter('weapp_mini_programe_options', function($options){
	if($weapps	= wpjam_get_setting('wpjam-content-template', 'weapps') ?: []){
		$options	= array_merge($options, wp_list_pluck($weapps, 'appid', 'name'));
	}
