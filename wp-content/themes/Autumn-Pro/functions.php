...
function wpdaxue_old_content_message($content) {
	...
	if($diffTime > 0.0001 && in_category(array(40,167,719,670,1002,844,719
,972,567,695,571,570)) ){
		$content = '<div class="old-message">最近一次更新于'.get_the_modified_time('Y年n月j日 l G:i').'，若该剧未及时更新后续资源或者资源你应该知道在哪通知督促补更！</div>'.$content;
	}
	return $content;
}
...
$current = current_time('timestamp');
$diffTime = ($current - $modified) / (60 * 60 * 24);
if ($diffTime > 0.0001 && in_category(array(1,168)) ) {
$content = '<div class="old-message">最近一次更新于'.get_the_modified_time('Y年n月j日').
'，若该影片资源失效，你知道在哪里反馈！</div>'.$content;
}
return $content;
}
...
function exclude_category_home( $query ) {
    if ( $query->is_home ) {
        $query->set( 'cat', '-344,-526,-1048,-1157' );
    }
    return $query;
}
