...
class WPJAM_Custom{
	...
	public static function migrate(){
		if(!wpjam_get_option('wpjam-custom')){
			...
			foreach (['admin_logo', 'admin_head', 'admin_footer', 'head', 'footer', 'login_head', 'login_footer', 'login_redirect'] as $name){
				if($value	= wpjam_basic_get_setting($name)){
					$custom_value[$name]	= $value;
				}

				wpjam_basic_delete_setting($name);
			}
	...
	public static function filter_admin_footer_text($text){
		$admin_footer	= self::get_instance()->get_setting('admin_footer');
		return $admin_footer ?: $text;
	}
