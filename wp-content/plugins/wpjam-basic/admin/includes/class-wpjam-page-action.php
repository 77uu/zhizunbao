...
class WPJAM_Page_Action{
	...
	public static function ajax_response(){
		...
		if($instance = self::get_instance($action)){
			...
			$ajax_response	= apply_filters('wpjam_page_ajax_response', $ajax_response, $GLOBALS['plugin_page'], $action, $action_type);
	...
	public static function ajax_form($args){
		...
		if(is_callable($args['fields'])){
			$fields	= call_user_func($args['fields'], $action);
	...
	public function get_button($args=[]){
		return self::ajax_button(array_merge($this->args, $args));
	}

	public function get_form($args=[]){
		return self::ajax_form(array_merge($this->args, $args));
	}
}
