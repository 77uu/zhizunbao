...
class WPJAM_Builtin_Page{
	...
	public static function load(){
		if($page_instance	= self::get_instance()){
			$page_instance->page_load();
		}
	...
	public static function get_instance(){
		...
		if(in_array($screen_base, ['term', 'edit-tags'])){
			return WPJAM_Term_Page::get_instance();
		}elseif(in_array($screen_base, ['edit', 'upload', 'post'])){
			return WPJAM_Post_Page::get_instance();
		}elseif($screen_base == 'users'){
			return WPJAM_User_Page::get_instance();
		}
...
class WPJAM_Post_Page{
	...
	protected function __construct(){
		$this->post_type	= $GLOBALS['current_screen']->base == 'upload' ? 'attachment' : $GLOBALS['current_screen']->post_type ?? '';
	...
	public function on_save_post($post_id, $post){
		...
		$post_data	= apply_filters('wpjam_save_post_fields', $post_data, $post_id);
...
class WPJAM_Term_Page{
	...
	protected function __construct(){
		$this->taxonomy	= $GLOBALS['current_screen']->taxonomy ?? '';
		$this->tax_obj	= get_taxonomy($this->taxonomy);
	}

	public function page_load(){
		$this->add_style();
		$this->term_options	= wpjam_get_term_options($this->taxonomy);

		if($this->term_options){
			if(wp_doing_ajax()){
	...
	public function get_term_fields($action='add'){
		return array_filter($this->term_options, function($field) use($action){ 
			return empty($field['action']) || $field['action'] == $action;
		});
	}

	public function on_add_form_fields($taxonomy){
		if($fields = $this->get_term_fields('add')){
			wpjam_fields($fields, [
				'fields_type'	=> 'div',
	...
	public function on_edit_form_fields($term){
		if($fields = $this->get_term_fields('edit')){
			wpjam_fields($fields, [
				'fields_type'	=> 'tr',
				'item_class'	=> 'form-field',
				'id'			=> $term->term_id,
