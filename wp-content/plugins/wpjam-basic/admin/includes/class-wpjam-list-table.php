...
class WPJAM_List_Table extends WP_List_Table{
	...
	public function __construct($args=[]){
		...
		$args['actions']	= array_merge($args['actions'], WPJAM_List_Table_Action::get_all());
	...
	protected function filter_list_action_result($result, $list_action, $id, $data){
		...
		return apply_filters($hook, $result, $list_action, $id, $data, $this->_args['singular']);
	}
	...
	public function ajax_form($list_action, $args=[]){
		...
		if($bulk){
			...
			$fields_args['id']			= $id;
			$fields_args['data_type']	= $this->_args['data_type'];
			...
			if($id && ($args['action_type'] != 'submit' || $args['response_type'] != 'form')){
				...
				if(empty($data) || is_wp_error($data)){
					wpjam_send_json(['errcode'=>'invalid_id', 'errmsg'=>'无效的ID']);
				}
			}
		}
	...
	public function get_fields($key='', $id=0, $args=[]){
		...
		if($key){
			...
			$fields	= wpjam_sort_items($this->filter_fileds($fields, $key, $id)+WPJAM_List_Table_Column::get_all());
		}
...
class WPJAM_List_Table_Action{
	...
	public static function register($key, $args){
		if(isset(self::$actions[$key])){
			trigger_error('List Table Action 「'.$key.'」已经注册。');
		}

		if(!isset($args['screen_id']) || $args['screen_id'] == get_current_screen()->id){
...
class WPJAM_List_Table_Column{
	...
	public static function register($key, $field){
		if(isset(self::$columns[$key])){
			trigger_error('List Table Column 「'.$key.'」已经注册。');
		}

		if(!isset($field['screen_id']) || $field['screen_id'] == get_current_screen()->id){
			self::$columns[$key]	= wp_parse_args($field, ['type'=>'view', 'show_admin_column'=>true]);
		}
	}

	public static function unregister($key){
		unset(self::$columns[$key]);
	}
