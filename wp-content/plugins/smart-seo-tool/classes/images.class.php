...
class Smart_SEO_Tool_Images {
	...
	public function __construct(){
	    ...
	    $this->content = Smart_SEO_Tool_Admin::cnf('img_seo.content');
	    $this->thumb = Smart_SEO_Tool_Admin::cnf('img_seo.thumb');
	    $this->type = Smart_SEO_Tool_Admin::cnf('img_seo.active');
	...
	public function handel_images( $content ) {
        ...
        $search = array('site_name'=>'','name'=>'','title'=>'','post_cat'=>'','num'=>'');
        if(preg_match('#%site_name#',$this->content)){
            $search['site_name'] = get_bloginfo('name', 'display');
        }
        ...
        foreach($match[1] as $k=>$img){
            ...
            if(!preg_match('#\s+title=.+?#is',$img)){
                ...
                if(preg_match('#/.+?\.(jpg|jpeg|gif|webp|png|bmp)#is',$img,$name_match)){
                    $search['name'] = basename($name_match[0]);
                }
    ...
    public function handel_images_featured( $html,$post_id ) {
        ...
        $search = array('site_name'=>'','name'=>'','title'=>'','post_cat'=>'','num'=>'');
        if(preg_match('#%site_name#',$this->thumb)){
            $search['site_name'] = get_bloginfo('name', 'display');
        }
